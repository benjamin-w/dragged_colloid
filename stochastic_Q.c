...
void evolveB(double** phi, double** y_colloid, long** neighbours, parameter* params, observables* obvs){
	...
	for(tstep = -thermal_steps; tstep < n_timestep; tstep++){									// Time evolution
		...
		for(i=0; i< DIM; i++){														// Evolve each of the components separately
			...
			stochastic_Q += ( (*y_colloid[i] -Y[i]) * ( 0.5*(F1[i]+F2)*params->delta_t ) ); // Q = (\dot{y} - \xi) \circ dy_t, I hope this is the correct Stratonovich way of doing things?
			delta_y += (*y_colloid[i] - Y[i]);
		}
		printf("#Q %.8f\t%.8f\n", stochastic_Q, delta_y);
	}
}
// Model A evolution
